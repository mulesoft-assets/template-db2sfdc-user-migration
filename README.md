...
This Template should serve as a foundation for the process of migrating users from database to one Salesforce instance, being able to specify filtering criteria and desired behavior if an user already exists in the destination org. 

As implemented, this Template leverages the [Batch Module](http://www.mulesoft.org/documentation/display/current/Batch+Processing).
The batch job is divided in Process Records and On Complete stages.
Firstly the Template hit the Database and query all existing Users matching the filter criteria.
...
Finally during the On Complete stage the Template will both output statistics data into the console and send notification email with the results of the batch execution.
...
+ Configure Database Connector in Global Elements section of the config flow to use your database specific driver.
...
+ Optionally customize query templates in `businessLogic.xml` if they don't match sql grammar of database of your choice
